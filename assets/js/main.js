(function () {
  ...
  function isHome() {
    // Anything not matching known sections is treated as Home
    if (path.includes("/orientation")) return false;
    if (path.includes("/downloads")) return false;
    if (path.includes("/support")) return false;
    if (path.includes("/sessions/")) return false;
    return true;
  }
  ...
  shell.innerHTML = `
    ...
    <div class="container layout ${home ? "layout--home" : ""}">
      <aside class="sidebar ${home ? "sidebar--hidden" : ""}" aria-label="Course navigation">
    ...
    <!-- Floating Quick Links pill -->
    <div class="quicklinks" data-quicklinks>
      <button class="quicklinks-btn" type="button" aria-expanded="false" aria-controls="quicklinks-panel">
        Quick links
      </button>

      <div class="quicklinks-panel" id="quicklinks-panel" aria-hidden="true">
        <a class="quicklinks-link" href="${prefix}">Home</a>
        <a class="quicklinks-link" href="${prefix}orientation/">Orientation</a>
        <a class="quicklinks-link" href="${prefix}sessions/session-1/">Session 1</a>
        <a class="quicklinks-link" href="${prefix}downloads/">Downloads</a>
        <a class="quicklinks-link" href="${prefix}support/">Support and contact</a>
      </div>
    </div>
